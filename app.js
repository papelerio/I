...
document.querySelectorAll('.anchor-btn').forEach(b => {
    b.onclick = () => {
        ...
        document.querySelectorAll('.anchor-btn').forEach(x => x.classList.remove('active'));
...
function showSelectionButtons(tool) {
    ...
    [lassoSelBtn, lassoDesBtn, modifySelBtn, clearSelBtn, bucketModeBtn].forEach(b => { if (b) b.classList.add('hidden'); });
...
function handlePointerDown(e) {
    ...
    if (currentTool === 'rotate') { rotationPivot = { sx: e.offsetX, sy: e.offsetY, startRotation: viewRotation }; }
        ...
        if (currentBrush.isLasso) lassoPath.push({ x: world.x, y: world.y });
        else drawPoint(lastX, lastY, smoothedPressure);
    }
    e.preventDefault();
}
...
function handlePointerUp(e) {
    ...
    if (currentTool === 'lazo-sel') {
        ...
        if (currentBrush.isLasso) executeLassoFill();
